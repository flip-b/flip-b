...
export default class AccountsController extends Controller {
  ...
  async search(body: any, auth: any): Promise<any> {
    ...
    // Define scope filter
    filters.$and.push({ _id: { $in: auth.accounts.map((v: any) => this.app.database.Types.ObjectId(v)) } });
    ...
    // Define result
    const result: any = await this.app.models.accounts.find(filters, select, options).exec();
    return result;
  }
  ...
  async count(body: any, auth: any): Promise<any> {
    ...
    // Define object
    const object: any = await this.app.models.accounts.aggregate([{ $match: filters }, { $facet: facets }]);
  ...
  async select(body: any, auth: any): Promise<any> {
    ...
    // Define object
    const object: any = await this.app.models.accounts.findOne(filters).select(select);
  ...
  async update(body: any, auth: any): Promise<any> {
    ...
    // Define object
    const object: any = body._id ? await this.app.models.accounts.findOne(filters) : new this.app.models.accounts();
    if (object) {
      for (const b in body) {
  ...
  async delete(body: any, auth: any): Promise<any> {
    ...
    // Define object
    const object: any = await this.app.models.accounts.deleteOne(filters);
